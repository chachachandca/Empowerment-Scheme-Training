...
type Tab = "dashboard" | "applicants" | "statistics" | "admins";
type AdminRow = { id: string; username: string; role: string; created_at: string };
...
export default function AdminDashboard() {
  ...
  const [newAdminPwShow, setNewAdminPwShow] = useState(false);
  ...
  const fetchAdmins = async () => {
    ...
    try {
      const base = import.meta.env.BASE_URL.replace(/\/$/, "");
      const res = await fetch(`${base}/api/admins`, { credentials: "include" });
      const data = await res.json() as AdminRow[] | { error: string };
      if (!res.ok) throw new Error((data as { error: string }).error ?? "Failed to load admins");
      setAdminsList(data as AdminRow[]);
    } catch (err) {
  ...
  const handleAddAdmin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newAdmin.username) { toast({ title: "Email is required", variant: "destructive" }); return; }
    if (newAdmin.password.length < 6) { toast({ title: "Password must be at least 6 characters", variant: "destructive" }); return; }
    ...
    try {
      ...
      const data = await res.json() as AdminRow | { error: string };
      if (!res.ok) throw new Error((data as { error: string }).error ?? "Failed to add admin");
      toast({ title: "Admin added successfully" });
      setShowAddAdmin(false);
      setNewAdmin({ username: "", password: "", role: "admin" });
      setAdminsList(prev => [...prev, data as AdminRow]);
    } catch (err) {
      toast({ title: "Failed to add admin", description: (err as Error).message, variant: "destructive" });
    } finally {
  ...
  const handleDeleteAdmin = async (admin: AdminRow) => {
    ...
    try {
      const base = import.meta.env.BASE_URL.replace(/\/$/, "");
      const res = await fetch(`${base}/api/admins/${admin.id}`, { method: "DELETE", credentials: "include" });
      const data = await res.json() as { error?: string };
      if (!res.ok) throw new Error(data.error ?? "Failed to remove admin");
      toast({ title: "Admin removed" });
      setAdminsList(prev => prev.filter(a => a.id !== admin.id));
    } catch (err) {
      toast({ title: "Failed to remove admin", description: (err as Error).message, variant: "destructive" });
    }
  };
  ...
  return (
    <div className="min-h-screen bg-muted/20 flex flex-col">
      ...
      <div className="flex flex-1 max-w-7xl mx-auto w-full px-4 py-6 gap-6">
        ...
        <aside className="hidden md:flex flex-col w-48 flex-shrink-0 gap-1">
          {([
            ...
            <button key={id} onClick={() => { setTab(id); if (id === "admins") fetchAdmins(); }}
        ...
        <div className="md:hidden w-full">
          <div className="flex gap-1 mb-4 bg-card border border-border rounded-lg p-1 flex-wrap">
            {([
              ["dashboard", "Dashboard", LayoutDashboard],
              ["applicants", "Applicants", Users],
              ["statistics", "Statistics", BarChart2],
            ...
            <div className="space-y-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-0.5">Manage who has access to this portal</p>
                </div>
              ...
              {adminsLoading ? (
                <div className="space-y-3">
                  {[...Array(3)].map((_, i) => <div key={i} className="bg-card border border-border rounded-xl p-4 h-16 animate-pulse bg-muted/40" />)}
                </div>
              ) : adminsList.length === 0 ? (
                <div className="bg-card border border-border rounded-xl p-10 text-center">
                  ...
                  <p className="text-sm text-muted-foreground/70 mt-1">Add an admin to grant portal access</p>
                </div>
              ) : (
                <div className="bg-card border border-border rounded-xl overflow-hidden">
                  <table className="w-full text-sm">
                    <thead className="border-b border-border bg-muted/30">
                      <tr>
                        {["Email / Username", "Role", "Added On", ""].map(h => (
                    ...
                    <tbody>
                      {adminsList.map(admin => {
                        const isSelf = admin.username === ((me as Record<string, unknown>).username as string);
                        return (
                          <tr key={admin.id} className="border-b border-border/50 last:border-0 hover:bg-muted/20 transition-colors">
                            <td className="py-3.5 px-4">
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  ...
                                  {isSelf && <span className="ml-2 text-xs bg-primary/10 text-primary px-1.5 py-0.5 rounded-full">You</span>}
                                </div>
                              </div>
                            </td>
                            <td className="py-3.5 px-4">
                              <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-secondary/20 text-secondary-foreground capitalize">{admin.role}</span>
                            </td>
                            <td className="py-3.5 px-4 text-muted-foreground">
                              {admin.created_at ? new Date(admin.created_at).toLocaleDateString("en-NG", { year: "numeric", month: "short", day: "numeric" }) : "—"}
                            </td>
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
                <strong>Note:</strong> Admins can view all applicants, run exports, and manage other admin accounts. Only add trusted personnel.
              </div>
            </div>
          )}
        </main>
      ...
      {/* ── ADD ADMIN MODAL ── */}
      {showAddAdmin && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setShowAddAdmin(false)}>
          <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-border">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-lg text-foreground">Add New Admin</h3>
              </div>
              <button onClick={() => setShowAddAdmin(false)} className="text-muted-foreground hover:text-foreground p-1">
                <X className="w-5 h-5" />
              </button>
            </div>
            <form onSubmit={handleAddAdmin} className="p-5 space-y-4">
              <p className="text-sm text-muted-foreground">This person will be able to log into the admin portal with the credentials you set below.</p>
              ...
              <div>
                ...
                <div className="relative">
                  <Input
                    type={newAdminPwShow ? "text" : "password"}
                  ...
                  />
                  <button type="button" onClick={() => setNewAdminPwShow(v => !v)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
                    {newAdminPwShow ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                  </button>
                </div>
                {newAdmin.password.length > 0 && newAdmin.password.length < 6 && (
              ...
              <div className="flex gap-3 pt-2">
                <Button type="button" variant="outline" className="flex-1" onClick={() => { setShowAddAdmin(false); setNewAdmin({ username: "", password: "", role: "admin" }); }}>
                  Cancel
                </Button>
                <Button type="submit" className="flex-1" disabled={addAdminLoading}>
